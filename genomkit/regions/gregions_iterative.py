...
# GRegions
###########################################################################
class GRegionsIterative(GRegions):
    """
    GRegions module
    ...
    def extend(self, upstream: int = 0, downstream: int = 0, strandness: bool = False, inplace: bool = True):
        ...
        if inplace:
            for region in self.elements:
                region.extend(upstream=upstream, downstream=downstream, strandness=strandness, inplace=True)
        else:
    ...
    def extend_fold(
        self, upstream: float = 0.0, downstream: float = 0.0, strandness: bool = False, inplace: bool = True
    ):
        """Perform extend step for every element. The extension length can also
        be negative values which shrinkages the regions.
        ...
        if inplace:
            for region in self.elements:
                region.extend_fold(upstream=upstream, downstream=downstream, strandness=strandness, inplace=True)
        else:
            output = GRegions(name=self.name)
            for region in self.elements:
            ...
            if not by_name and not strandness:
                for cur_region in self.elements[1:]:
                    if prev_region.overlap(cur_region):
                        prev_region.start = min(prev_region.start, cur_region.start)
                        prev_region.end = max(prev_region.end, cur_region.end)
                    else:
                        res.add(prev_region)
                ...
                for cur_region in self.elements[1:]:
                    if (
                        (prev_region.overlap(cur_region))
                        and (prev_region.name == cur_region.name)
                        and (prev_region.orientation == cur_region.orientation)
                    ):
                        prev_region.start = min(prev_region.start, cur_region.start)
                        prev_region.end = max(prev_region.end, cur_region.end)
                    else:
    ...
    def intersect_array(self, target, strandness: bool = False):
        super().intersect_array(target)

        def find_intersects(orientation):
            array_1 = self.get_array_by_seq(sequence=seq, orientation=orientation)
            array_2 = target.get_array_by_seq(sequence=seq, orientation=orientation)
            ...
            ranges = [(indices[i - 1], indices[i]) for i in range(1, len(indices), 2)]
            return ranges
        ...
        def make_array_same_length(array_1, array_2):
            ...
            if len(array_1) < max_length:
                array_1 = np.append(array_1, [False] * (max_length - len(array_1)))
            elif len(array_2) < max_length:
                array_2 = np.append(array_2, [False] * (max_length - len(array_2)))
        ...
        for seq in common_seq:
            ...
            for pair in ranges:
                res.add(GRegion(sequence=seq, start=pair[0], end=pair[1], name=""))
        return res
    ...
    def subtract(
        self, regions, whole_region: bool = False, merge: bool = True, exact: bool = False, inplace: bool = True
    ):
